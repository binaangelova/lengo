...
const AdminTestResults = () => {
  ...
  if (!testResults) {
    return (
      <div className="bg-blue-100 min-h-screen flex flex-col">
        <AdminNavbar />
        <div className="flex-grow flex flex-col items-center justify-center">
          <div className="text-xl text-red-600 mb-4">Не са намерени резултати</div>
          <button
            onClick={() => navigate("/admin/users")}
            className="px-6 py-2 bg-blue-600 text-white rounded-xl shadow-xl text-md font-semibold hover:bg-blue-800 transition duration-200"
          >
  ...
  const correctAnswersCount = testResults.questions.reduce((count, q, index) => {
    return count + (testResults.selectedAnswers[index] === testResults.correctAnswers[index] ? 1 : 0);
  }, 0);
  const score = Math.round((correctAnswersCount / testResults.questions.length) * 100);

  return (
    <div className="bg-blue-100 min-h-screen flex flex-col">
      <AdminNavbar />
      <div className="flex-grow flex flex-col items-center pt-16">
        <h1 className="text-4xl font-bold text-blue-900 drop-shadow-xl mb-4">{testResults.lessonName} - Резултати</h1>
